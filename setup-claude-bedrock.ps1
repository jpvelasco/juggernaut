...
# Usage: .\setup-claude-bedrock.ps1 [-Force] [-DryRun] [-Region <region>] [-Help]
...
# Show help
if ($Help) {
    Write-Host "Usage: .\setup-claude-bedrock.ps1 [-Force] [-DryRun] [-Region <region>] [-Help]"
    Write-Host ""
    Write-Host "Options:"
...
Write-Host "🚀 Setting up Claude Code with Amazon Bedrock..." -ForegroundColor Cyan

# Configuration block with start/end markers for reliable removal
$ConfigBlock = @"
...
Write-Host "📁 Target file: $ProfilePath" -ForegroundColor Gray

# Check if configuration already exists
$ProfileContent = Get-Content $ProfilePath -Raw -ErrorAction SilentlyContinue
if ($ProfileContent -match "CLAUDE_CODE_USE_BEDROCK") {
    Write-Host "⚠️  Configuration already exists in $ProfilePath" -ForegroundColor Yellow
    ...
    if ($DryRun) {
        ...
        Write-Host "🗑️  Removed existing configuration" -ForegroundColor Gray
    }
}
