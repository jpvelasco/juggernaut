...
func TestApply_PermissionMode_Plan_NoAutoModeEnvVar(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	env, _ := settings["env"].(map[string]any)
	if env["CLAUDE_CODE_ENABLE_AUTO_MODE"] == "1" {
		t.Error("CLAUDE_CODE_ENABLE_AUTO_MODE must NOT be set for non-auto permission modes")
	}
	perms, _ := settings["permissions"].(map[string]any)
	if perms["defaultMode"] != "plan" {
		t.Errorf("expected permissions.defaultMode=plan, got %v", perms["defaultMode"])
	}
}

func TestApply_ModelOverrides_WrittenAsTopLevelKey(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	overrides, ok := settings["modelOverrides"].(map[string]any)
	if !ok {
		t.Fatal("expected top-level modelOverrides key in settings.json")
	}
	if overrides["opus"] == nil || overrides["sonnet"] == nil || overrides["haiku"] == nil {
		t.Errorf("expected opus/sonnet/haiku in top-level modelOverrides, got %v", overrides)
	}
}

func TestApply_UninstallReApply_SkipWebFetchPreflight_Restored(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	if settings["skipWebFetchPreflight"] != true {
		t.Error("expected skipWebFetchPreflight=true to be restored after uninstall+re-apply")
	}
}
