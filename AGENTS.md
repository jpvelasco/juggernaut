...
Config paths: Claude `~/.claude/settings.json` (user) / `./.claude/settings.json` (project) — the only CLI with both scopes; Codex `~/.codex/config.toml` / `./.codex/config.toml` via the built-in `amazon-bedrock-runtime` provider (`[model_providers.amazon-bedrock-runtime.aws]` region table; requires Codex ≥ 0.153.4 — apply/doctor warn on older binaries); OpenCode `~/.config/opencode/opencode.json` / `./opencode.json` via `provider.amazon-bedrock` (region + discovered `models` + `whitelist`, omitted when discovery finds no supported models — never `null`/`[]`); Grok `~/.grok/config.toml` user-only (`base_url=https://bedrock-runtime.{region}.amazonaws.com/openai/v1`, model `global.xai.grok-4.6`; `auth_provider_command="juggernaut auth-token"` only for `--auth=bedrock-api-key`). Launch reads `juggernaut.auth.mode` from project then user for non-Claude CLIs (token-gated only for API-key mode).

**OpenCode auth-mode sidecar (OpenCode validates `opencode.json` against a strict schema, `additionalProperties: false`):** the `juggernaut.auth` block does NOT go in `opencode.json` — it goes to an owner-only sidecar next to the config, `./.juggernaut.json` (project) / `~/.config/opencode/.juggernaut.json` (user), holding `{"juggernaut": <auth block>}` (non-secret; the bearer token stays in the keychain). Codex/Grok keep the block in-file (TOML isn't schema-validated). `opencode.NativeManagedKeys` = `{model, provider}` (no `juggernaut`). `internal/provider` `SidecarAuthSource` + helpers (`WriteSidecar`, `ReadSidecarAuthMode`, `SidecarExists`, `RemoveSidecar`, `MigrateSidecarLegacy`) implement the extension; `resolveLaunchConfigPaths` lists sidecars (project, user) before configs. A plain `apply` strips a legacy in-file block (v6.2.0–v6.3.0) via `MigrateSidecarLegacy` under the config lock; `uninstall --cli=opencode` removes the sidecar and any leftover in-file block.
