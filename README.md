...
Each provider filters the live inventory by what its client protocol can use: Claude Code accepts `foundation`/`profile` Anthropic models, Codex accepts `foundation`/`profile` OpenAI GPT-5.6 (`sol`/`terra`/`luna`), OpenCode accepts `foundation`/`profile` OpenAI-compatible models (injected into its built-in `provider.amazon-bedrock` block with a `whitelist` of discovered models, omitted when none are found), and Grok accepts `foundation`/`profile` xAI Grok models over the native `bedrock-runtime` OpenAI-compatible endpoint (`https://bedrock-runtime.{region}.amazonaws.com/openai/v1`). This compatibility policy prevents advertising a model to a client that cannot speak its API while avoiding a maintained roster. Explicit `--model` selections remain supported and warn when the cached catalog says they are unavailable in the chosen region.

**OpenCode auth metadata:** OpenCode validates `opencode.json` against a strict schema that rejects unknown top-level keys, so Juggernaut writes the `juggernaut.auth` mode/region to a separate owner-only sidecar file next to the config (`./.juggernaut.json` for project scope, `~/.config/opencode/.juggernaut.json` for user scope) rather than inside `opencode.json`. The launch wrapper reads it the same way it reads the in-file block for other CLIs (project first, then user). A plain `apply` over an OpenCode config written by v6.2.0–v6.3.0 automatically strips the legacy in-file block and moves the auth mode to the sidecar.
