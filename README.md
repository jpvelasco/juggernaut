...
```bash
# Unix/macOS/Linux
curl -fsSL https://raw.githubusercontent.com/jpvelasco/juggernaut/v2.2.1/install.sh | bash -s -- --version v2.2.1
```

```powershell
# Windows PowerShell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/jpvelasco/juggernaut/v2.2.1/install.ps1))) -Version v2.2.1
```

**Manual clone:**

```bash
git clone --branch v2.2.1 --depth 1 https://github.com/jpvelasco/juggernaut.git && cd juggernaut

export JUGGERNAUT_USE_V2=1
./juggernaut apply
```

The pinned installer commands above install v2.2.1 directly from the release tag. Installers do not change your Claude configuration unless you explicitly run `juggernaut apply`.
...
Both scripts normalize the version automatically — `2.2.1` and `v2.2.1` both work.
