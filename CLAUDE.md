...
Codacy cloud analysis runs on every push/PR via the Codacy GitHub integration — `Codacy Static Code Analysis` is a required status check on `main`. Coverage is handed off from `ci.yml` as a run-scoped artifact and uploaded by the trusted `workflow_run` workflow only when that artifact exists; the uploader passes `workflow_run.head_sha` so coverage is associated with the tested revision and reads the repository secret `CODACY_REPOSITORY_API_TOKEN` without exposing it to PR code. Use `make codacy` to check dashboard issues locally (requires `@codacy/codacy-cloud-cli` + `CODACY_API_TOKEN`). The `.codacy/` directory holds synced tool configs (eslint, opengrep, pmd, trivy) and `.codacy.yml` excludes `.remember/`, `.codacy/`, and `mcps/` from analysis (`npm/` is additionally excluded from the eslint engine).
