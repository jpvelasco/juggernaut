...
- CI (`ci.yml`): lint job (golangci-lint + version sync check + GoReleaser draft check) → `go test ./... -v -race`-covered on ubuntu/macos/windows with per-OS Codecov upload via OIDC (merged for true cross-platform coverage, since much of the PowerShell/launcher/keychain code is Windows-only) → separate race-detector job → separate Linux coverage-threshold job (60% floor, JUnit uploaded to Codecov Test Analytics) → npm tests → shellcheck → gosec. The Ubuntu coverage job also saves `coverage.out` as a run-scoped artifact. The separate `codacy-coverage.yml` `workflow_run` workflow runs with trusted repository context, downloads that artifact when available, and uploads it to Codacy with `CODACY_REPOSITORY_API_TOKEN` plus the triggering run's `head_sha`; it never executes the PR revision. Actions pinned to full commit SHAs.
...
- Socket (`socket.yml`): supply-chain security scan on PRs to `main`; `firewall-free` mode (informational PR comments only, never blocks); pinned to `SocketDev/action@v1.3.2`.
...
Codacy is **informational**, not a merge or main CI gate. Cloud analysis still runs on every push/PR via the Codacy GitHub integration (grade badge, PR comments, dashboard), but `Codacy Static Code Analysis` is not a required status check and CI does not fail on outstanding dashboard issues.
...
Local dashboard queries (`make codacy`, `@codacy/codacy-cloud-cli`) use a personal **account** API token via `CODACY_API_TOKEN` on the developer's machine only. The `.codacy/` directory holds synced tool configs (eslint, opengrep, pmd, trivy) and `.codacy.yml` excludes `.remember/`, `.codacy/`, and `mcps/` from analysis (`npm/` is additionally excluded from the eslint engine).
