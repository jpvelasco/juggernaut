...
- **Suppressions are documented, not silent.** Every remaining `//nolint`, `#nosec`, and `// nosemgrep` annotation is listed with a one-line rationale in `docs/ci-suppressions.md`; prefer a small code adjustment over a suppression and never add one without documenting it there.
