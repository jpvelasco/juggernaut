...
- CI (`ci.yml`): lint + version sync check → `go test ./... -v` on ubuntu/macos/windows → race detector → coverage (60% threshold) → npm tests → shellcheck → gosec. Actions pinned to full commit SHAs. Go module cache is shared across jobs.
