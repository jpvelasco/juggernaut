...
- `activation/` — manages shell profile marker blocks, implements `juggernaut launch` (reads the configured storage backend at runtime to inject `ANTHROPIC_API_KEY`), resolves the real Anthropic `claude` binary while avoiding recursion, recovers only positively identified v4.2.6 launcher artifacts, and detects stale v3 installs via `DetectV3Install` (`v3detect.go`).
